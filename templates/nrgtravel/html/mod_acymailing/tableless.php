...
?><div class="<?php echo $params->get('moduleclass_sfx')?>" id="acymailing_module_<?php echo $formName; ?>">
<?php
	$style = array();
	if($params->get('effect','normal') == 'mootools-slide'){
		if(!empty($mootoolsIntro)) echo '<div class="text">'.$mootoolsIntro.'</div>'; ?>
	...
	?>
       <?php if ($module->showtitle) {
         echo '<h3 class="title">' .$module->title .'</h3>';
       } ?>
       <?php if(!empty($introText)) echo '<div class="text">'.$introText.'</div>'; ?>
	<div class="subscription-form acymailing_fulldiv" id="acymailing_fulldiv_<?php echo $formName; ?>" <?php echo $styleString; ?> >
		<form class="subscribe-form clearfix" id="<?php echo $formName; ?>" action="<?php echo JRoute::_('index.php'); ?>" onsubmit="return submitacymailingform('optin','<?php echo $formName;?>')" method="post" name="<?php echo $formName ?>" <?php if(!empty($fieldsClass->formoption)) echo $fieldsClass->formoption; ?> >

                    <label>SUBSCRIBE</label>
		<div class="acymailing_module_form input-container clearfix" >
			<?php 
			...
			<div class="acymailing_form">
					...
					foreach($fieldsToDisplay as $oneField){
						...
						if($oneField == 'name' AND empty($extraFields[$oneField])){
							...
							<span class="acyfield_<?php echo $oneField. (!$displayOutside? ' acy_requiredField':''); ?>"><input id="user_email_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'readonly="readonly" '; if(!$displayOutside){ ?> onfocus="if(this.value == '<?php echo $emailCaption;?>') this.value = '';" onblur="if(this.value=='') this.value='<?php echo $emailCaption?>';"<?php } ?> class="subscribe-email inputbox" type="text" name="user[email]" style="width:<?php echo $fieldsize; ?>" value="<?php if(!empty($identifiedUser->userid)) echo $identifiedUser->email; elseif(!$displayOutside) echo $emailCaption; ?>" title="<?php echo $emailCaption;?>" /></span>
							<?php
				...
				<?php }
				 ...
				 if($params->get('showterms',false)){
					...
					<p class="acysubbuttons">
						...
						<input class="subscribe-submit" type="submit" value="" name="Submit" onclick="try{ return submitacymailingform('optin','<?php echo $formName;?>'); }catch(err){alert('The form could not be submitted '+err);return false;}"/>
						...
						<input class="subscribe-submit" type="button" value="" name="Submit" onclick="return submitacymailingform('optout','<?php echo $formName;?>')"/>
						<?php } ?>
					</p>
				</div>
			<?php
